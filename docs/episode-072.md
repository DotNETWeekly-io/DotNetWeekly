...
3. 推广与落地  
   ...
   • 迁移成本小：IDE Code Fix + 文档指引，80% 以上项目在短期内完整升级。

4. 收获  
   • 代码评审更专注业务逻辑，样式争议显著减少，合并速度提升。  
   • 统一风格、性能与安全基准，提高整体代码质量；少量大型解决方案编译时间最多缩短 20%。  
...
4、 [在浏览器中运行 .NET 但不使用 Blazor](https://andrewlock.net/running-dotnet-in-the-browser-without-blazor/)
...
5、 [.NET 开发者学会的 Prompts](https://devblogs.microsoft.com/dotnet/5-copilot-chat-prompts-dotnet-devs-should-steal-today/)
...
Facet 是一个 C# Source Generator，允许开发者直接从领域模型在编译期产生轻量投影类型（DTO、ViewModel、API Model 等）。只需在目标类型上标注 `[Facet(typeof(Source))]`，即可自动生成带构造子、属性及可选 LINQ 投影的 partial class/record/struct，全程零执行期开销。

主要特性：
• 透过“Facetting”机制排除或保留属性，建立专注视图，消除手写 DTO、Mapper 与 Select 的重复代码。
• 生成 `Expression<Func<TSource,TTarget>>` 静态投影，可与 Entity Framework Core 等 LINQ Provider 无缝结合。
• 支持自定义同步 (`IFacetMapConfiguration`) 与异步 (`IFacetMapConfigurationAsync`) 映射，能利用 DI 注入外部服务并完整支持 CancellationToken。
• 提供平行处理与集合转换 API，兼顾性能与易用性。

生态系 NuGet 套件：
1. Facet：核心产生器与 API。
2. Facet.Extensions：与任何 LINQ Provider 通用的投影/映射扩展方法。
3. Facet.Extensions.EFCore：针对 EF Core 6+ 提供 `ToFacetsAsync`、`SelectFacets` 等异步扩展。
4. Facet.Mapping：进阶静态或 DI 驱动映射设定，支持同步、异步与平行转换。
...
```
若需自定义字段或 I/O 操作，可撰写对应的 Mapper 类别并于属性中指定；于 EF Core 查询链中使用 `ToFacetsAsync`，仅拉取必要字段以降低 IO，生成的 Expression 亦可被转换为最佳化 SQL。

总结：Facet 以编译期代码生成消弭样板，让 .NET 开发者能在保持类型安全与高效能的同时，快速建立各式 DTO/视图与映射流程。